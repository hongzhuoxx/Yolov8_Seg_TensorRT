...
void DrawPred(Mat& img, std::vector<OutputSeg> result) {
	...
	for (int i = 0; i < result.size(); i++) {
		...
		char label[100];
		sprintf_s(label, "%d:%.2f", result[i].id, result[i].confidence);
	...
	//将mask加在原图上面
	addWeighted(img, 0.5, mask, 0.8, 1, img); 
...
/// 实例分割
/// </summary>
/// <param name="enginePath">TensorRT 模型所在路径</param>
/// <param name="imgPath">要进行分割的图像路径</param>
/// <returns></returns>
int seg_trt_go(char* enginePath, char* imgPath)
{
	// create a model using the API directly and serialize it to a stream
	// 
	// char* trtModelStream==nullptr;  开辟空指针后 要和new配合使用，比如89行 trtModelStream = new char[size]
